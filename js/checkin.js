...
const STORAGE_KEY = 'montessori_math_checkin';
...
function getRewardProgress() {
  ...
  return {
    total,
    vipNeeded: Math.max(0, 27 - total),
    familyNeeded: Math.max(0, 24 - total),
    isVip: total >= 27,
    isFamily: total >= 24,
  };
}
...
function renderMyProgress() {
  ...
  const percent = Math.round((progress.total / 30) * 100);
  ...
  let rewardHtml = '';
  if (progress.isVip) {
    rewardHtml = `<div class="reward-status vip">🏆 VIP 달성! 축하합니다!</div>`;
  } else if (progress.isFamily) {
    rewardHtml = `<div class="reward-status family">⭐ 패밀리 달성! VIP까지 ${progress.vipNeeded}회 남음</div>`;
  } else {
    rewardHtml = `
      <div class="reward-status-list">
        <div class="reward-status-item">🏆 VIP까지 <strong>${progress.vipNeeded}회</strong></div>
        <div class="reward-status-item">⭐ 패밀리까지 <strong>${progress.familyNeeded}회</strong></div>
      </div>
    `;
  }
  ...
  container.innerHTML = `
    <div class="my-progress-card">
      <div class="my-progress-header">
        ...
        <div class="my-progress-count">${progress.total}/30 완료</div>
      </div>
